...
class CrudService 
{
    ...
    public static function form( $entry = null, $config = [] )
    {
        ...
        if(User::count() >= License::first()->users && $permissionType == 'create') {
          //  ns()->restrict([ $instance->getPermission( $permissionType ) ]);

           throw new NotAllowedException( __( 'You\'re not allowed to add more than :user users.', ['user' =>  License::first()->users] ) );
        }
